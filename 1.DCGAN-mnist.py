...
class GAN:
	...
	def __init__(self, sess):
		...
		with tf.name_scope("generate_image_from_noise"):
			self.Gen = self.Generator(self.noise_source) #batch_size, self.height, self.width, self.channel
		...
		with tf.name_scope("result_from_Discriminator"):
			...
			self.D_Gen, self.D_Gen_logits = self.Discriminator(self.Gen, True) #batch_size, 1, 1, 1
	...
	def Discriminator(self, data, reuse=False): #batch_size, 1, 1, 1
		with tf.variable_scope('Discriminator') as scope:
			...
			#input layer는 BN 안함.
			conv0 = tf.layers.conv2d(inputs=data, filters=128, kernel_size=[4, 4], strides=(2, 2), padding='same') #batch, 32, 32, 128
	...
	#노이즈로 진짜같은 데이터 생성
	def Generator(self, noise): #batch_size, self.height, self.width, self.channel
#if padding == same
	...
	# shape = [batch, (input_height*stride_left) + (kerner_size_left-stride_left) , (input_width*stride_right) + (kerner_right-stride_right), filter] 
		...
		with tf.variable_scope('Generator'):
			...
			conv0 = tf.layers.conv2d_transpose(inputs=noise, filters=1024, kernel_size=[4, 4], strides=(1, 1), padding='valid') #batch, 4, 4, 1024
...
def train(model, data):
	...
	for i in range( iteration ):
		...
		#Discriminator 학습.
		_, D_loss = sess.run([model.D_minimize, model.D_loss], {model.X:input_, model.noise_source:noise, model.is_train:True})

		#Generator 학습. 		#batch_normalization을 하기 때문에 X data도 넣어줘야함.
		_, G_loss = sess.run([model.G_minimize, model.G_loss], {model.X:input_, model.noise_source:noise, model.is_train:True})
...
def gen_image(model, epoch):
	...
	generated = sess.run(model.Gen, {model.noise_source:noise, model.is_train:False}) #num_generate, 64, 64, 1
...
def run(model, train_set, restore = 0):
	...
	for epoch in range(restore + 1, 10001):
		...
		D_loss, G_loss = train(model, train_set)
